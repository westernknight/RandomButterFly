...
public class KinectPlayerAnalyst : MonoBehaviour
{
    ...
    public bool isKinectInitialized = false;

    // The singleton instance of KinectManager
    public static KinectPlayerAnalyst instance = null;
    ...
    public bool isCanUpdateAvatar = false;
    ...
    // List of all users
    private List<Int64> alUserIds = new List<long>();
    private Dictionary<Int64, int> dictUserIdToIndex = new Dictionary<long, int>();
    ...
    {
        return instance != null ? instance.isKinectInitialized : false;
    }

    // checks if Kinect is initialized and ready to use. If not, there was an error during Kinect-sensor initialization
    public bool IsInitialized()
    {
        return isKinectInitialized;
    }
    ...
    {
        return isKinectInitialized && (alUserIds.Count > 0);
    }
    ...
    public Vector3 GetRightHandPosition(Int64 userId)
    {
        if (dictUserIdToIndex.ContainsKey(userId))
        {
            int index = dictUserIdToIndex[userId];
                ...
                bodyFrame.bodyData[index].bIsTracked != 0)
            {
                ...
                return bodyData.joint[(int)JointType.HandRight].position;
            }
        }
        return Vector3.zero;
    }
    ...
    public Vector3 GetLeftHandPosition(Int64 userId)
    {
        if (dictUserIdToIndex.ContainsKey(userId))
        {
                ...
                bodyFrame.bodyData[index].bIsTracked != 0)
            {
                ...
                return bodyData.joint[(int)JointType.HandLeft].position;
            }
        }
        return Vector3.zero;
    }
    ...
    IEnumerator InitKinect()
    {
        ...
        if (sensorData == null)
        {
            Debug.Log("OpenDefaultKinectSensor failed");
        }
